...
import { Alert, Modal, StyleSheet, Text, Platform, View, TextInput, } from 'react-native';
...
const CreateCustomer: React.FC<Props> = (props) => {
  ...
  const [formItem, setFormItem] = useState<Omit<CustomerEntity, 'id' | 'created_at' | 'modified_at'>>(defaultCustomerEntity)
  ...
  const handleSave = async (values: Omit<CustomerEntity, 'id' | 'created_at' | 'modified_at'>) => {
    if (isEdit) {
      if (!_.isNil(customerInfo)) {
      ...
      >
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={styles.title}>{`${isEdit ? 'Update' : 'Create'} Customer`}</Text>
                    ...
                    <View style={styles.section}>
                      ...
                      <View style={styles.checkboxContainer}>
                        <Checkbox
                          ...
                          onValueChange={(value) => setFormItem((pre) => ({ ...pre, is_employee: value }))}
                          color={formItem.is_employee ? '#4630EB' : undefined}
                        />
                      </View>
                    </View>

                    {loading ? <ActivityIndicator animating={true} color={MD2Colors.red800} /> : <Button onPress={() => handleSubmit()}>Save</Button>}
                  </View>
                )}
              </Formik>
            )}
